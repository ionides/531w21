...
\title{\link{https://ionides.github.io/531w21/}{Analysis of Time Series (STATS/DATASCI 531, Winter 2021)}\\ \vspace{2mm}
...
\item You can get this file from the course website, \link{http://ionides.github.io/531w21}{ionides.github.io/531w21}. 

\item Better, you can make a local clone of this git repository that will give you an up-to-date copy of all the data, notes, code, homeworks and solutions for this course. See Homework 0.
...
\item We use lower case Roman letters ($x$, $y$, $z$, $\dots$) to denote numbers. These are not random variables. Often, $y$ will denote a data point.
...
We see that the two estimates, $\estimate{\hat\mu_1}=\Sexpr{round(mu1,2)}$ and $\estimate{\hat\mu_2}=\Sexpr{round(mu2,2)}$, are close. 

\begin{itemize}

\item However, $\SE_1=\Sexpr{formatC(round(se1,2),2,format="f")}$ is an underestimate of error, compared to the better estimate $\SE_2=\Sexpr{formatC(round(se2,2),2,format="f")}$.

\item The naive standard error needs to be inflated by $100(\SE_2/\SE_1-1)=$ \Sexpr{round((se2/se1 - 1)*100,1)} percent.

\end{itemize}

Exactly how the ARMA(1,1) model is fitted and the standard errors computed will be covered later.

\myquestion. How do we know if the ARMA analysis is more trustworthy?
...
\item For an ARMA model, the residual $\data{r_n}$ at time $t_n$ is defined to be the difference between the data, $\data{y_n}$, and a one-step ahead prediction of $\data{y_n}$ based on $\data{y_{1:n-1}}$, written as $\predict{y_n}$.
\end{itemize}
...
We see slow variation in the residuals, over a decadal time scale. However, the residuals $\data{r_{1:N}}$ are close to uncorrelated. We see this by plotting their pairwise sample correlations at a range of lags. This is the \myemph{sample autocorrelation function}, or \myemph{sample ACF}, written for each lag $h$ as
...
\item The \myemph{return} is the difference of the log of the index. If $z_n$ is the index value for day $n$, then $y_n = \log(z_n)-\log(z_{n-1})$.
...
\begin{itemize}
\item By the end of this course, you will be able to carry out data analysis developing complex models and fitting them to time series. See past final projects for \link{https://ionides.github.io/531w16/final_project/}{2016}, \link{https://ionides.github.io/531w18/final_project/}{2018} and \link{https://ionides.github.io/531w20/final_project/}{2020}.
\end{itemize}
\end{frame}
...
\begin{frame}{License, acknowledgments, and links}
  \begin{itemize}
  \item
  \parbox[t]{0.75\textwidth}{Licensed under the \link{http://creativecommons.org/licenses/by-nc/4.0/}{Creative Commons Attribution-NonCommercial license}.
