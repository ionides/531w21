...
\item We think of unemployment as a \myemph{proxy variable} for economic fluctuations. We do not claim that increased unemployment itself is necessarily directly causing reduced mortality. All proxy variables for economic activity are confounded in this analysis.
...
\item  Researchers have searched unsuccessfully for a stable lagged relationship between economic activity and life expectancy.

\item We have found empirical evidence to support a claim that above-trend economic growth CAUSES above-trend mortality.

\item We have NOT found evidence that unemployment reduces mortality for those who lose their jobs. Adverse individual-level effects of unemployment can be reconciled with our result \citep{tapia14}.
...
\bi
\item Temporal differencing has destroyed much of the evidence we were hoping it might clarify.
