...
\section{Cross-correlation, coherence and phase}
...
\item $\hat\rho_{xy}(h)$ estimates $\rho_{XY}(h)=\mathrm{Cor}\big(X_{n+h},Y_n\big)$, the cross-correlation at lag $h$ for a bivariate stationary time series model $\big(X_{1:N},Y_{1:N}\big)$.
...
\bi
\item The Fourier transform of the sample cross-correlation is called the \myemph{coherency}.
