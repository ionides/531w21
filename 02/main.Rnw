...
Chapter \CHAPTER: Time series models, trend and autocovariance}
...
\item For a covariance stationary model, and some mean estimate $\hat\mu_n=\hat \mu_n(y_{1:N})$, an estimate for $\gamma_h$ is the \myemph{sample autocorrelation function},
...
\section{Least squares estimation of a linear trend}
...
\section{Least squares estimation with a quadratic trend}
