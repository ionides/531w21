...
\begin{frame}{Introduction to smoothing}
...
\bi
\item
We can wonder how the Bureau of Labor Statistics adjusts the data, and if this introduces any artifacts that a careful statistician should be aware of.
\ei
\end{frame}
...
\subsection{The frequency response function}
...
\subsection{The frequency response function for loess}
...
We plot a smoothed periodogram for the raw unemployment data. We cut out higher frequencies to zoom in on frequencies relevant to our business cycle investigation. 
