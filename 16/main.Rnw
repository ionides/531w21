...
Chapter \CHAPTER: A case study of financial volatility and a POMP model with observation-dependent dynamics}
...
\section{Introduction to time series models for financial volatility}
...
\begin{frame}[fragile]{Financial leverage}
